...
function animateThemePicker(nextTheme) {
  ...
  themePickerAnimationTimeoutId = setTimeout(clearThemePickerAnimation, 400);
}
...
function replayTextAnimations() {
  ...
  document.querySelectorAll(".text-with-animation span").forEach((span, i) => {
    ...
    span.style.animation =
      textAnimations[i] ?? textAnimations[textAnimations.length - 1];
  });

  // index.html: .subtext-with-animation spans use word-animation-1
  // Staggered to enter 1s after first element finishes
  const subtextAnimations = [
    "word-animation-1 0.5s ease-out 1.1s forwards",
    "word-animation-1 0.5s ease-out 1.4s forwards",
  ];
  document
    .querySelectorAll(".subtext-with-animation span")
    .forEach((span, i) => {
      span.style.animation = "none";
      void span.offsetWidth; // force reflow so the browser restarts the animation
      span.style.animation =
  ...
  document
    ...
    .forEach((span, i) => {
      ...
      void span.offsetWidth; // force reflow so the browser restarts the animation
      span.style.animation =
        subtext1Animations[i] ??
        subtext1Animations[subtext1Animations.length - 1];
    });
}
...
function setTheme(theme) {
    ...
    try {
      ...
      transition.finished
        .then(() => {
          updateFavicon(resolvedTheme);
          setActiveThemeButton(theme);
          setTimeout(replayTextAnimations, 400); // delay text animations until theme settles
          endTransition(2900);
        })
        .catch(() => {
          document.documentElement.classList.remove(THEME_TRANSITION_CLASS);
  ...
  applyTheme(resolvedTheme, theme);
  setTimeout(replayTextAnimations, 400); // delay text animations until theme color transitions settle
  endTransition(700);
}
...
function bindThemeButtons() {
  document.querySelectorAll("[data-theme-toggle]").forEach((button) => {
    ...
    button.addEventListener("click", (e) =>
      setTheme(e.currentTarget.dataset.themeToggle),
    );
...
function bindOverlayNavigation() {
  ...
  const { signal } = (openOverlay._overlayAbortController =
    new AbortController());
  ...
  const handler = () => {
    ...
    if (!overlayNavigation) {
      ...
      document.querySelectorAll("nav li").forEach((item, i) => {
        if (i < NAV_OPEN_CLASSES.length)
          item.classList.add(NAV_OPEN_CLASSES[i]);
      });
      return;
    }
...
document.addEventListener("page:transitioned", () => {
  ...
  initPage();
  setTimeout(replayTextAnimations, 100); // fire text animations on the new page
});
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  updateFavicon(resolvedTheme);
  initPage();
  setTimeout(replayTextAnimations, 100); // fire text animations after first paint
});
