...
document.querySelectorAll('.crew-details .menu-icon').forEach(menuIcon => {
  menuIcon.addEventListener('click', function () {
    // Hide the current panel
    const panel = menuIcon.closest('.crew-details');
    if (panel) panel.classList.remove('show');

    // Show the navigation menu
    const menuToggle = document.querySelector('.burger-check'); // or '.check' if that's your toggle
    const profileNavbar = document.querySelector('.navigation');
